...
let ptrAtualizaTela;
let ptrCriaBlocoVerde;

ptrAtualizaTela = setInterval(atualizaTela, 50);
ptrFuncUpdate = setInterval(funcUpdate, 1000);
//ptrCriaBlocoVerde = setInterval(criarBlocoVerde, 1000);
...
function funcUpdate() {
    ...
    updateServer(channel, "move", player + "A" + x + "A" + y);
}
...
function updateServer(channel, event, message) {
    const xhr = new XMLHttpRequest();

    const params = "channel=" + channel + "&event=" + event + "&message=" + message;
...
function atualizaTela() {
    ...
    //updateServer(localStorage.getItem("channel") + localStorage.getItem("player"), "move", x + "A" + y);

    //Atualiza Bloco Inimigo
	const blocoInimigo = document.getElementById("blocoInimigo");

	blocoInimigo.style.marginLeft = (xI + "px");
	blocoInimigo.style.marginTop = (yI + "px");
